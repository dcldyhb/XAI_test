...
# --- 0. 关键：引入项目根目录以加载 config ---
current_dir = os.path.dirname(os.path.abspath(__file__))
# 向上两级: PySR_gamma -> XAI_test (项目根目录)
...
# --- 1. 定义输出目录 ---
# 使用 config 中的路径，不再硬编码
print(f"所有输出结果将被保存在目录: '{PYSR_OUTPUT_DIR}'\n")
os.makedirs(PYSR_OUTPUT_DIR, exist_ok=True)

# --- 2. 数据加载与预处理 ---
try:
    print(f"正在读取数据: {DATASET_PATH}")
    df = pd.read_csv(DATASET_PATH)
    print(f"成功加载数据。\n")
except FileNotFoundError:
    print(f"错误: 找不到文件 at '{DATASET_PATH}'。请检查路径是否正确。")
    exit()
...
# 生成 Python 模块的头部
# 关键修改：让生成的代码也导入项目 config 来获取 DEVICE
python_code_lines = [
...
# 注意：这里假设输入的 state 是一个 tensor 或者 numpy array
# 如果 state 是 tensor，我们通常需要先转成 list 或者逐个索引
# PySR 原生使用的是 x0, x1 这种变量名
# 这里我们假设 state 已经是一个可以按索引访问的对象 (如 numpy array 或 list)
for i, name in enumerate(feature_names):
    # 如果 state 是 tensor，提取标量值
...
gamma_csv_path = os.path.join(PYSR_OUTPUT_DIR, "gamma_formulas.csv")
try:
...
# --- 写入 result.md ---
markdown_file_path = os.path.join(PYSR_OUTPUT_DIR, "result.md")
try:
